...
class WebBot{
    ...
    */
    static registerMain(webMain, ip, port, options = {}, driverFolder = "../"){
        ...
        WebBot.servers.push(server);
        server.listen(port);
        server.on('connection', (clientSocket) => {
            server.clientList.push(clientSocket);
            webMain(new WebBot(clientSocket, server));
        });
    }
    ...
    */
     ...
     * @param {string} filePath 本地文件路径，路径不存在会导致网页崩溃。同时上传多个文件 使用竖线 "|" 分割多个文件路径
     * @return {Promise.<boolean>} 成功返回true，失败返回false
    */
    ...
    */
    ...
    /**获取页面所有元素信息
     * @param {{left:number, top:number, right:number, bottom:number}|null} rectFilter 过滤 不在指定区域中的元素，默认为 null 不过滤。
     * @return {Promise.<{elements:[{}]}|null>} 成功返回json格式的元素信息，失败返回null
    */
    async getElements(rectFilter = null){
        let strData = this.setSendData("getElements");
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        try{
            let result = JSON.parse(strRet);

            //过滤 不在指定矩形区域的 其他元素
            if(rectFilter != null && rectFilter != ''){
                result = {
                    elements: result.elements.filter(el => {
                        return (
                            el.rect.left >= rectFilter.left &&
                            el.rect.top >= rectFilter.top &&
                            el.rect.right <= rectFilter.right &&
                            el.rect.bottom <= rectFilter.bottom
                        );
                    })
                };
            }
    ...
    /**绝对XPath 转换 相对XPath
     * @param {string} absoluteXPath 绝对XPath
     * @return {Promise.<string>} 返回相对XPath
    */
    async convertToRelativeXPath(absoluteXPath){
        let strData = this.setSendData("convertToRelativeXPath", absoluteXPath);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        return strRet;    
    }
    ...
    */
    ...
    /**启用网络数据监听，此函数一般在goto、点击跳转等操作 前调用。可调用多次监听多个目标
     * @param {string} url 监听的url。支持 “*” 星号模糊匹配。例如：https://www.aibote.net/a1/b2/* 匹配 https://www.aibote.net/a1/b1/c1 
     * @param {string} method 监听的请求类型。'GET'或'POST'
     * @return {Promise.<boolean>} 总是返回 true
    */
    async enableNetwork(url, method){
        let strData = this.setSendData("enableNetwork", url, method);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == "false")
            return false;
    ...
    /**等待网络数据，此函数必须在 goto、点击跳转等操作后立刻执行(goto、点击跳转 操作不能有 await 等待)。执行完自动关闭 enableNetwork 设置的监听目标
     * @param {number} timeout  等待超时,单位毫秒。默认10秒
     * @return {Promise.<[{url:string, method:string, data:string}]}>} 成功返回jsons数组格式的数据，失败返回null。
    */
    async waitNetworkData(timeout = 10000){
        let strData = this.setSendData("waitNetworkData", timeout);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == "")
            return null;

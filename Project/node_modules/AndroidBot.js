...
class AndroidBot{
    ...
    */
    static registerMain(androidMain, port = 16678){
        ...
        AndroidBot.servers.push(server);
        server.listen(port);
        server.on('connection', (clientSocket) => {
            server.clientList.push(clientSocket);
            androidMain(new AndroidBot(clientSocket, server));
        });
    }
    ...
    */
    async getElements(rectFilter = null, ocrServerIp = null){
        ...
        try{
            ...
            if (rectFilter != null && rectFilter !== '') {
                ...
                if (result.words) {
                    result.words = result.words.filter(el => (
                        el.box[0] >= rectFilter.left &&
                        el.box[1] >= rectFilter.top &&
                        el.box[2] <= rectFilter.right &&
                        el.box[3] <= rectFilter.bottom
                    ));
                }
            }
    ...
    * @param {string} androidFilePath 安卓文件保存路径, 安卓外部存储根目录 /storage/emulated/0/  媒体文件建议存放至 /storage/emulated/0/Android/media/com.aibot.client/ 目录下
    * @return {Promise.<boolean>} 成功返回true 失败返回false
    */
    ...
    */
    ...
    startWindowsBot = () =>{
        ...
        const server = new net.createServer();
        server.clientList = [];//添加 clientList 属性
        WindowsBot.servers.push(server);
